...
public class OrderAdapterShipperDaNhan extends RecyclerView.Adapter<OrderAdapterShipperDaNhan.ViewHolder>{

    List<OrderHistory> arrayOrder;
    ApiShopLapTop apiShopLapTop;
    private AppCompatActivity mActivity;
    public OrderAdapterShipperDaNhan(List<OrderHistory> arrayOrder, AppCompatActivity activity) { this.arrayOrder = arrayOrder;  mActivity = activity;}
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnCreateContextMenuListener, View.OnLongClickListener {
        Button btAcceptOrder,btXemOrder,btCancelOrder;
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            btCancelOrder = itemView.findViewById(R.id.btCancelOrderAdmin);
...
//            itemClickListener.onItemClick();
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View inflate = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_order_shipper_da_nhan, parent,false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.btAcceptOrder.setOnClickListener(view -> {
            apiShopLapTop = RetrofitClient.getInstance(Utils.BASE_URL).create(ApiShopLapTop.class);
            int _id = arrayOrder.get(position).getOrder_id();
            Call<OrderModel> call = apiShopLapTop.updateStatusShipperGiaoThanhCong(_id);
            call.enqueue(new Callback<OrderModel>() {
                @Override
                public void onResponse(Call<OrderModel> call, Response<OrderModel> response) {
                    OrderModel laptopModel = response.body();
                    if (laptopModel.isSuccess()) {
                        Toast.makeText(view.getContext(), "Đơn hàng giao thành công.", Toast.LENGTH_LONG).show();
...
//
        ...
        holder.btCancelOrder.setOnClickListener(view -> {
            apiShopLapTop = RetrofitClient.getInstance(Utils.BASE_URL).create(ApiShopLapTop.class);
            int _id = arrayOrder.get(position).getOrder_id();
            Call<OrderModel> call = apiShopLapTop.updateStatusShipperGiaoKhongThanhCong(_id);
            call.enqueue(new Callback<OrderModel>() {
                @Override
                public void onResponse(Call<OrderModel> call, Response<OrderModel> response) {
                    OrderModel laptopModel = response.body();
                    if (laptopModel.isSuccess()) {
                        Toast.makeText(view.getContext(), "Đơn hàng giao không thành công.", Toast.LENGTH_LONG).show();
//                        Intent intent = new Intent(view.getContext(), DonDaXacNhanActivity.class);
//                        view.getContext().startActivity(intent);
//                        if (!mActivity.isFinishing()) {
//                            mActivity.finish();
